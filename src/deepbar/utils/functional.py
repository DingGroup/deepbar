...
def _compute_particle_xyz_from_internal(
    particle_1_xyz, particle_2_xyz, particle_3_xyz, bond, angle, dihedral
):
...
def _compute_distances(xyz, particle_index):
...
def _compute_angles(xyz, particle_index):
...
def _compute_dihedrals(xyz, particle_index):
    ...
    b1_cross_b2 = torch.linalg.cross(b1, b2)
    b3_cross_b2 = torch.linalg.cross(b3, b2)

    cos_d = torch.norm(b2, dim=-1) * torch.sum(b1_cross_b2 * b3_cross_b2, -1)
    sin_d = torch.sum(b2 * torch.linalg.cross(b3_cross_b2, b1_cross_b2), -1)
...
def _compute_polar_angle(xyz, particle_index):
...
def _compute_azimuthal_angle(xyz, particle_index):
