...
for idx in range(M**2):
    ...
    trajs.append(traj)
traj = mdtraj.join(trajs, check_topology = False)
...
res = fastmbar.calculate_free_energies_of_perturbed_states(energy_PMF)
PMF = res['F']
...
plt.ylim(-180, 180)
plt.tight_layout()
plt.savefig("./output/umbrella_sampling/PMF_contourf.png")
