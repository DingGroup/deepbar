...
coor_transformer = MMFlow.utils.CoordinateTransformer(
    particle_bonds, reference_particle_1, reference_particle_2, reference_particle_3
)
...
with torch.no_grad():
    ic, ic_logabsdet = coor_transformer.compute_internal_coordinate_from_xyz(
        torch.from_numpy(traj.xyz)
    )
...
with open(file_name, "wb") as file_handle:
    pickle.dump(
        {"ic": ic, "prob": prob_md, "coor_transformer": coor_transformer}, file_handle
    )
